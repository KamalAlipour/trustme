...
export function CreditRequestForm() {
  ...
  const submitLoan = async () => {
    ...
    try {
      ...
      await request('/v1/me/loans', { method: 'POST', body: { principalCoupons: amount, installments: rows, guarantors: selectedGuarantors.map((barcodeId) => ({ barcodeId, amountCoupons: amount })) } });
      setAmount(''); setSelectedGuarantors([]); setInstallments([{ dueAt: new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10), amountCoupons: '' }]); await invalidate();
  ...
  return (
    <View style={styles.card}>
      ...
      {(contacts.data?.items ?? []).map((contact) => {
        ...
        return <Pressable key={contact.id} onPress={() => { setCharityId(''); setSelectedGuarantors(selected ? selectedGuarantors.filter((barcodeId) => barcodeId !== contact.barcodeId) : [...selectedGuarantors, contact.barcodeId]); }}>
