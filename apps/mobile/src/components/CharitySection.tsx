...
export function CharitySection() {
  ...
  const sendDocuments = async (requestId: string, documentMediaIds: string[]): Promise<boolean> => {
    try { await request(`/v1/me/aid-requests/${requestId}/documents`, { method: 'POST', body: { mediaIds: documentMediaIds } }); await invalidate(); return true; } catch (cause) { setError(mapApiError(cause)); return false; }
  };
  const review = async (item: AidRequest, action: 'approve' | 'reject' | 'request-documents') => {
    ...
    {(agent.data?.items ?? []).length > 0 ? <><Text style={styles.heading}>درخواست‌های مددکاری</Text>{(agent.data?.items ?? []).map((item) => <View key={item.id} style={styles.card}>
      <Text style={styles.heading}>{item.applicant?.displayName ?? item.applicant?.barcodeId}</Text><Text style={styles.text}>{formatCoupons(item.amountCoupons)} کوپن · {item.description}</Text><EvidenceViewer ids={item.mediaIds} />
