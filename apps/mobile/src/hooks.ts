...
import type { AidRequest, Balance, BalanceDisclosure, Charity, CharityGuarantee, Contact, EscrowBalance, EscrowConfig, EscrowPermitDeposit, EscrowSettlement, EscrowUnload, IdentityInfo, Loan, Member, RefundRequest, TransactionsPage, WithdrawalAvailability } from './api/types';
...
export function useCharityGuarantees() { return useQuery({ queryKey: ['charity-guarantees'], queryFn: () => request<{ items: CharityGuarantee[] }>('/v1/me/charity-guarantees') }); }
...
export function useEscrowPermitDeposits(enabled = true) { return useQuery({ queryKey: ['escrow-permit-deposits'], queryFn: () => request<{ items: EscrowPermitDeposit[]; nextCursor: string | null }>('/v1/me/escrow/permit-deposits'), enabled, refetchInterval: 10_000 }); }
export function useInvalidateMoney() {
  const client = useQueryClient();
  return () => Promise.all([client.invalidateQueries({ queryKey: ['me'] }), client.invalidateQueries({ queryKey: ['identity'] }), client.invalidateQueries({ queryKey: ['disclosures'] }), client.invalidateQueries({ queryKey: ['balance'] }), client.invalidateQueries({ queryKey: ['withdrawal-availability'] }), client.invalidateQueries({ queryKey: ['transactions'] }), client.invalidateQueries({ queryKey: ['loans'] }), client.invalidateQueries({ queryKey: ['refunds'] }), client.invalidateQueries({ queryKey: ['aid-requests'] }), client.invalidateQueries({ queryKey: ['charity-requests'] }), client.invalidateQueries({ queryKey: ['charity-guarantees'] }), client.invalidateQueries({ queryKey: ['escrow-balance'] }), client.invalidateQueries({ queryKey: ['escrow-settlements'] }), client.invalidateQueries({ queryKey: ['escrow-unloads'] }), client.invalidateQueries({ queryKey: ['escrow-permit-deposits'] })]);
}
