...
export const apiConfigSchema = z.object({
  ...
  escrowUsdtPermitName: z.string().default('USDT0'),
  escrowUsdtPermitVersion: z.string().default('1'),
  escrowNativeCurrencySymbol: z.string().default('POL'),
  escrowChainName: z.string().default('Polygon Mainnet'),
  walletConnectProjectId: z.string().optional(),
  web3AuthClientId: z.string().optional(),
  transakApiKey: z.string().optional(),
  transakApiSecret: z.string().optional(),
  transakEnvironment: z.enum(['staging', 'production']).default('staging'),
  transakReferrerDomain: z.string().default('app-trustcoupon.komasi.as'),
  transakSellRedirectUrl: z.string().url().default('https://app-trustcoupon.komasi.as/tether'),
  ...
  vippsMsn: z.string().optional(),
  vippsApiBase: z.string().url().default('https://api.vipps.no'),
  vippsScope: z.string().default('openid name phoneNumber email'),
  vippsRedirectUri: z.string().url().default('https://api-trustme.komasi.as/v1/me/identity/vipps/callback'),
  vippsReturnUrl: z.string().url().default('https://app-trustcoupon.komasi.as/profile'),
...
export function loadApiConfig(env: NodeJS.ProcessEnv = process.env): ApiConfig {
  const config = apiConfigSchema.parse({
    ...
    escrowUsdtPermitName: env.ESCROW_USDT_PERMIT_NAME,
    escrowUsdtPermitVersion: env.ESCROW_USDT_PERMIT_VERSION,
    escrowNativeCurrencySymbol: env.ESCROW_NATIVE_CURRENCY_SYMBOL,
    escrowChainName: env.ESCROW_CHAIN_NAME,
    walletConnectProjectId: env.WALLETCONNECT_PROJECT_ID || undefined,
    web3AuthClientId: env.WEB3AUTH_CLIENT_ID || undefined,
    transakApiKey: env.TRANSAK_API_KEY || undefined,
    transakApiSecret: env.TRANSAK_API_SECRET || undefined,
    transakEnvironment: env.TRANSAK_ENVIRONMENT,
    transakReferrerDomain: env.TRANSAK_REFERRER_DOMAIN,
    transakSellRedirectUrl: env.TRANSAK_SELL_REDIRECT_URL,
    ...
    vippsClientId: env.VIPPS_CLIENT_ID || undefined,
    vippsClientSecret: env.VIPPS_CLIENT_SECRET || undefined,
    vippsSubscriptionKey: env.VIPPS_SUBSCRIPTION_KEY || undefined,
    vippsMsn: env.VIPPS_MSN || undefined,
    vippsApiBase: env.VIPPS_API_BASE,
    vippsScope: env.VIPPS_SCOPE,
    vippsRedirectUri: env.VIPPS_REDIRECT_URI,
    vippsReturnUrl: env.VIPPS_RETURN_URL,
