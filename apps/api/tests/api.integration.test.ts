...
import { AccountType, AdminRole, Asset, BalanceDisclosureStatus, PrismaClient, TransactionType, WithdrawalStatus } from '@trustme/db';
...
describe('public reserves and balance disclosure API', () => {
  ...
  it('expires requests, denies pending owner requests, and returns confirmation history once', async () => {
    ...
    const oldestAt = new Date(historyNow - 2_000);
    const newestAt = new Date(historyNow - 1_000);
    const heldAt = new Date(historyNow);
    const oldestTransaction = await prisma.transaction.create({ data: { type: TransactionType.TRANSFER, status: 'COMPLETED', amountCoupons: 25n, externalRef: 'public-history-oldest', createdAt: oldestAt } });
    const newestTransaction = await prisma.transaction.create({ data: { type: TransactionType.TRANSFER, status: 'COMPLETED', amountCoupons: 50n, externalRef: 'public-history-newest', createdAt: newestAt } });
    const escrowAccount = await account(AccountType.ESCROW, Asset.COUPON);
    const heldTransaction = await prisma.transaction.create({ data: { type: TransactionType.ESCROW_HOLD, status: 'CONFIRMED', amountCoupons: 10n, externalRef: 'public-history-held', createdAt: heldAt } });
    await prisma.ledgerEntry.create({ data: { transactionId: oldestTransaction.id, fromAccountId: systems.issuance.id, toAccountId: ownerAccount.id, amount: 25n, asset: Asset.COUPON, createdAt: oldestAt } });
    await prisma.ledgerEntry.create({ data: { transactionId: newestTransaction.id, fromAccountId: systems.issuance.id, toAccountId: ownerAccount.id, amount: 50n, asset: Asset.COUPON, createdAt: newestAt } });
    await prisma.ledgerEntry.create({ data: { transactionId: heldTransaction.id, fromAccountId: ownerAccount.id, toAccountId: escrowAccount.id, amount: 10n, asset: Asset.COUPON, createdAt: heldAt } });
